...
OBJLoader.prototype = {
  ...
  load: function(url, onLoad, onProgress, onError) {
    ...
    scope.basePath = path.dirname(url);
    scope.mtlPath = undefined;
    ...
    loader.load(
      ...
      function(text) {
        ...
        if (scope.mtlPath) {
          mtlLoader.setTexturePath(scope.basePath + "/");
          mtlLoader.load(scope.mtlPath, materials => {
            materials.preload();
            var meshes = result.children;
            for (var i = 0; i < meshes.length; i++) {
              var mesh = meshes[i];
              mesh.material = materials.create(mesh.material.name);
            }
          });
        }
  ...
  parse: function(text) {
    ...
    for (var i = 0, l = lines.length; i < l; i++) {
      ...
      if (lineFirstChar === "v") {
        ...
        // mtl file
        var mtlSrc = line.substring(7).trim();
        state.materialLibraries.push(mtlSrc);
        scope.mtlPath = path.resolve(scope.basePath, mtlSrc);
